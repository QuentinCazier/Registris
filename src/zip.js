...
export function lireZip(source) {
  ...
  for (let n = 0; n < nombre; n++) {
    if (tampon.readUInt32LE(position) !== SIGNATURE_CENTRALE) throw new Error('archive ZIP invalide : répertoire corrompu');
    ...
    if (tampon.readUInt32LE(decalageLocal) !== SIGNATURE_LOCALE) throw new Error(`archive ZIP invalide : en-tête local manquant pour ${nom}`);
    const debut = decalageLocal + 30 + tampon.readUInt16LE(decalageLocal + 26) + tampon.readUInt16LE(decalageLocal + 28);
    ...
    else if (methode === 8) entrees.set(nom, zlib.inflateRawSync(donnees));
